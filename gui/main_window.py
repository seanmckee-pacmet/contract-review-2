...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QListWidget, QPushButton, QLabel, 
                             ...
                             QApplication, QMainWindow, QTreeWidget, QTreeWidgetItem, QMenu, QDialog, QDialogButtonBox)
...
class ReviewThread(QThread):
    ...
    def __init__(self, jobs, selected_documents):
        super().__init__()
        self.jobs = jobs
    ...
    def run(self):
        ...
        for company_name, file_paths in self.jobs.items():
            results[company_name] = review_documents(file_paths, company_name, self.selected_documents)
...
class MainWindow(QWidget):
    ...
    def review_all_jobs(self):
        ...
        selected_documents = self.select_documents_for_review()
        if not selected_documents:
            self.loading_indicator.hide()
            self.setEnabled(True)
            return

        self.review_thread = ReviewThread(self.jobs, selected_documents)
    ...
    def select_documents_for_review(self):
        ...
        if not documents:
            QMessageBox.warning(self, "Error", "No documents found for this company.")
            return

        dialog = QDialog(self)
        dialog.setWindowTitle("Select Documents for Review")
        layout = QVBoxLayout(dialog)

        document_list = QListWidget()
        document_list.addItems(documents)
        document_list.setSelectionMode(QListWidget.MultiSelection)
        ...
        if dialog.exec_() == QDialog.Accepted:
            selected_documents = [item.text() for item in document_list.selectedItems()]
